...
(defn pronunciation
  "Return the pronunciation of a sequence of numbers"
  [s]
  (->> (partition-by identity s)
       (mapcat #(vector (count %) (first %)))))

(defn pronunciations
  "Return an infinite lazy sequence of pronunciations of an initial sequence of numbers"
  [s]
  (rest (iterate pronunciation s)))

(def __ pronunciations)

(comment
  (apply concat (pronunciation [2 1])))
