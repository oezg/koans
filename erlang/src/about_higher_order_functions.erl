...
-export([map_a_list_to_a_new_list/0, fold_an_answer_from_the_left/0, or_from_the_right/0,
         filter_the_appropriate_values/0, merging_two_dicts/0, function_passed_to_a_function/0]).
...
fold_an_answer_from_the_left() ->
    "hello erlang koans"
    =:= lists:foldl(fun(Item, Accumulation) -> string:join([Accumulation, Item], " ") end,
                    "hello",
                    ["erlang", "koans"]).

or_from_the_right() ->
    "hello koans erlang"
    =:= lists:foldr(fun(Item, Accumulation) -> string:join([Accumulation, Item], " ") end,
                    "hello",
                    ["erlang", "koans"]).
