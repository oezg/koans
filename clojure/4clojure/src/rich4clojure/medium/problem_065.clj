...
(def restricted [class type Class vector? sequential? list? seq? map? set? instance?])

(defn sequence-type [coll]
  (let [e (conj (empty coll) [:a 1] [:b 2] [:b 2])]
    (cond
      (= 1 (:a e)) :map
      (= 2 (count e)) :set
      (= 1 (second (first e))) :vector
      (= 2 (second (first e))) :list)))

(def __ sequence-type)
